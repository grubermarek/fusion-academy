...
async function seedData() {
  ...
  // (booking flow odpočítaval aj pri online/free day — opravené, tu vrátime vstupy)
  if(!(await q.one(db.settings,{key:'fix_online_entry_refund_v1'}))){
    await q.insert(db.settings,{key:'fix_online_entry_refund_v1', value:true, at:nowISO()});
    try{
      const classes=await q.find(db.classes,{});
      const onlineIds=new Set(classes.filter(c=>c.category==='Online').map(c=>c._id));
      const bookings=(await q.find(db.bookings,{})).filter(b=>onlineIds.has(b.class_id)
        && ['single_entry','free_credit'].includes(b.access_method) && b.status!=='cancelled');
      const byUser={};
      for(const b of bookings) (byUser[b.user_id]=byUser[b.user_id]||[]).push(b);
      for(const [uid,bs] of Object.entries(byUser)){
        const u=await q.one(db.users,{_id:uid}); if(!u) continue;
        const nEntry=bs.filter(b=>b.access_method==='single_entry').length;
        const nCredit=bs.filter(b=>b.access_method==='free_credit').length;
        const set={};
        if(nEntry) set.single_entries=(u.single_entries||0)+nEntry;
        if(nCredit) set.free_credits=(u.free_credits||0)+nCredit;
        await q.update(db.users,{_id:uid},{$set:set});
        for(const b of bs) await q.update(db.bookings,{_id:b._id},{$set:{access_method:'online_refunded'}});
        await q.insert(db.notifications,{user_id:uid,type:'credit',
          title:'🎟️ Vrátili sme ti vstup',
          body:`Rezervácia online hodiny ti omylom odpočítala ${nEntry+nCredit>1?(nEntry+nCredit)+' vstupy/kredity':'1 vstup'}. Vrátili sme ti ${nEntry?nEntry+'× vstup z permanentky':''}${nEntry&&nCredit?' a ':''}${nCredit?nCredit+'× kredit':''}. Prepáč a ďakujeme za trpezlivosť! 💛`,
          read:false, created_at:nowISO()}).catch(()=>{});
        console.log('🎟️ ONLINE REFUND:', u.name, '+'+nEntry+' vstupov, +'+nCredit+' kreditov');
      }
    }catch(e){ console.error('online refund migration:', e.message); }
  }

  // ── Zápis 10.8.: Lenka Horáková a Ivka Jasenská platili 10 € hotovosť za vstup ──
  if(!(await q.one(db.settings,{key:'record_cash_entries_20260810'}))){
    await q.insert(db.settings,{key:'record_cash_entries_20260810', value:true, at:nowISO()});
    try{
      for(const nm of ['Horákov','Jasensk']){
        const u=(await q.find(db.users,{})).find(x=>new RegExp(nm,'i').test(x.name||'') && !/@test-fa-qa\.local$/.test(x.email||''));
        if(!u){ console.log('💶 CASH ENTRY: nenašiel som klientku '+nm); continue; }
        const dup=(await q.find(db.transactions,{user_id:u._id})).find(t=>t.created_at?.slice(0,10)==='2026-08-10' && t.amount===10);
        if(dup){ console.log('💶 CASH ENTRY: '+u.name+' už má dnes zapísaných 10 € — preskakujem'); continue; }
        await q.insert(db.transactions,{type:'single_entry', user_id:u._id, user_name:u.name, amount:10,
          payment_method:'cash', note:'Jednorazový vstup — hotovosť (dodatočný zápis 10.8.)',
        ...
        const todayB=(await q.find(db.bookings,{user_id:u._id})).find(b=>b.booking_date==='2026-08-10' && b.status!=='cancelled');
        if(todayB) await q.update(db.bookings,{_id:todayB._id},{$set:{entry_collected:{amount:10,method:'cash',at:nowISO(),by:'migration'}, pay_on_site:false}});
        await q.insert(db.notifications,{user_id:u._id,type:'payment',
          title:'🧾 Potvrdenie o platbe — vstup',
          body:'Prijali sme 10.00 € v hotovosti za dnešný vstup. Ďakujeme!',
          read:false, created_at:nowISO()}).catch(()=>{});
...
app.post('/api/register', rlSignup, async(req,res)=>{
  try {
    ...
    let vencekClass=null, vencekRole='student';
    ...
    if(vencekClass){
      ...
      try{ const admins=await q.find(db.users,{is_admin:true});
        for(const a of admins) await q.insert(db.notifications,{user_id:a._id,type:'venceky',
          title: ['teacher','director'].includes(vencekRole)?'⏳ Venčeky: čaká na schválenie':'🎓 Nový venčekár',
          body:`${name} (${roleLbl}) — trieda ${vencekClass.name}${vencekRole==='parent'&&req.body.vencek_child_name?' · dieťa: '+String(req.body.vencek_child_name).slice(0,80):''}`,
          read:false, created_at:nowISO()}); }catch(e){}
      if(['teacher','director'].includes(vencekRole)){
        await q.insert(db.notifications,{user_id:u._id,type:'venceky',
          title:'⏳ Účet čaká na schválenie',
...
app.get('/api/profile/:id', auth, async(req,res)=>{
  try {
    ...
    // Admin môže dátum „odkedy tancuje" nastaviť ručne (member_since) — má prednosť
    // pred automaticky odvodeným (import z Glofoxu nie vždy preniesol správny začiatok).
    const joinedDate = (u.member_since||'').slice(0,10)
      || ((_membStarts[0] && _membStarts[0] < (u.created_at||'9999').slice(0,10))
...
app.post('/api/membership/buy', auth, async(req,res)=>{
  try {
    ...
    await q.insert(db.notifications,{user_id:u._id,type:'membership',
      ...
      read:false, created_at:nowISO()}).catch(()=>{});
    res.json({ok:true, credit_used:creditUsed, promo_discount:promoDiscount, final_price:finalPrice, payment_id:manualPay._id,
      message:'Žiadosť odoslaná! Zaplať v hotovosti na hodine alebo prevodom — aktivujeme hneď po prijatí platby.'});
  } catch(e){ res.status(500).json({error:e.message}); }
});
...
app.get('/api/admin/trainers/performance', adminAuth, async(req,res)=>{
  try {
    ...
    // Skutočný inštruktor termínu — rešpektuje jednorazové výmeny (session_instructors),
    // inak tréner, ktorý zaskakuje (napr. Nelka), v štatistike vôbec nefiguruje.
    const insOverride=await sessionInstructorMap();
    const instructorOf=b=>(insOverride[b.class_id+'|'+bDate(b)]?.name)||clsMap[b.class_id]?.instructor||'—';
    ...
    const trainers=[...new Set([...classes.map(c=>c.instructor), ...Object.values(insOverride).map(o=>o?.name)].filter(Boolean))].sort();
...
app.get('/api/admin/class-history', adminAuth, async(req,res)=>{
  try {
    ...
    for(const b of bookings){
      ...
      const cls=clsMap[b.class_id];
      // Online hodiny netvoria výplatu ani tržbu hodín (členky majú online v členstve) —
      // rovnaké pravidlo ako v trainerMonthStats, inak by sa tu ukazovali falošné zárobky.
      if(cls?.category==='Online' || /online/i.test(cls?.location||b.class_location||'')) continue;
...
async function trainerMonthPayslip(trainerUser, month){
  ...
  const tips=await trainerTipsForMonth(trainerUser._id, month).catch(()=>0);
  const priv=await privateEarningsFor(trainerUser._id, month).catch(()=>({amount:0,count:0}));
  ...
  const lines=[
    ...
    ['💛 Tipy od klientov (80 %)', '', tips],
    ['🎭 Súkromné hodiny (môj podiel)', priv.count?`${priv.count}×`:'', +priv.amount],
  ].filter(l=>l[2]>0);
  if(ded>0) lines.push(['Zrážky', '', -ded]);
  if(cash.deduct>0) lines.push(['Vybraná hotovosť od klientov (nechávaš si)', '', -cash.deduct]);
  return {tName, email:trainerUser.email, month, st, base, bonuses, billable, affiliate, tips, private:priv.amount, ded, total, lines};
}
...
app.post('/api/admin/payouts/pay', adminAuth, async(req,res)=>{
  try {
    ...
    const paidBefore=+prevPayments.reduce((s,p)=>s+(+p.amount||0),0).toFixed(2);
    ...
    const histEntry={at:nowISO(), actor:req._auditActor||null, change:{status:payMeta.status,payment_method:method,amount,paid_total,total}, reason:fullyPaid?'Uzavretie mesiaca a vyplatenie':'Čiastočné vyplatenie'};
...
// Čakajúce manuálne platby (hotovosť/prevod z cenníka) — potvrdením sa aktivuje členstvo/permanentka
app.get('/api/admin/manual-payments', adminAuth, async(req,res)=>{
  try{
    const pend=await q.find(db.payments,{status:'pending_manual'});
    const users=await q.find(db.users,{});
...
app.post('/api/admin/manual-payments/:id/confirm', adminAuth, async(req,res)=>{
  try{
    ...
    createInvoice({user_id:p.user_id, client_name:buyer?.name, client_email:buyer?.email,
      items:[{desc:plan.type==='bundle'?plan.name:('Členstvo '+plan.name), qty:1, total:p.amount}],
      total:p.amount, method:methodTxt});
    awardPurchaseCommission({buyer_id:p.user_id, amount:p.amount, product_name:plan.type==='bundle'?plan.name:('Členstvo '+plan.name)});
    res.json({ok:true});
  }catch(e){ res.status(500).json({error:e.message}); }
});
app.post('/api/admin/manual-payments/:id/cancel', adminAuth, async(req,res)=>{
  try{
    const p=await q.one(db.payments,{_id:req.params.id, status:'pending_manual'});
...
// Zľava na súkromné hodiny podľa aktívneho členstva: Bronze −10 %, Silver −20 %, Gold −30 %
const PRIVATE_MEMBER_DISCOUNT = { bronze:10, silver:20, gold:30 };
async function privateDiscountFor(userId){
  const m = await checkMembership(userId);
  if(!m || m.status!=='active') return { pct:0, plan:null };
  const pct = PRIVATE_MEMBER_DISCOUNT[m.plan_id] || 0;
  return { pct, plan: pct ? (m.plan_name||m.plan_id) : null };
}
function privDiscounted(price, pct){ return +((+price||0)*(1-(pct||0)/100)).toFixed(2); }
...
app.get('/api/private/slots', auth, async(req,res)=>{
  try{
    ...
    const disc=await privateDiscountFor(req.session.uid); // zľava podľa členstva klienta
    res.json({ok:true, discount_pct:disc.pct, discount_plan:disc.plan,
      slots:slots.map(s=>({id:s._id,date:s.date,time_start:s.time_start,duration_min:s.duration_min||60,city:s.city,location:s.location||'',price:s.price,my_price:privDiscounted(s.price,disc.pct)}))});
  }catch(e){ res.status(500).json({error:e.message}); }
});
  ...
  try{
    ...
    if(u.email) sendMail(u.email,'🎭 Súkromná hodina rezervovaná — Fusion Academy',
      emailTemplate('Rezervácia potvrdená',
        `<p>Ahoj <b>${u.name.split(' ')[0]}</b>,</p><p>tvoja súkromná hodina je rezervovaná:</p><ul style="color:#ccc"><li>🎓 Tréner: <b>${s.trainer_name}</b></li><li>📅 ${when}</li><li>⏱ ${s.duration_min||60} min</li><li>💶 ${price.toFixed(2)} €${disc.pct?` (so zľavou ${disc.pct} % — ${disc.plan})`:``} ${pay==='credit'?'(zaplatené kreditom)':'(platba na mieste)'}</li></ul><p><b>Storno podmienky:</b> bezplatné zrušenie najneskôr 24 hodín pred začiatkom. Pri neskoršom zrušení suma prepadá trénerovi.</p>`,
        '📱 Moje rezervácie',`${APP_URL}/client-dashboard`)).catch(()=>{});
    res.json({ok:true, id:bk._id, price, pay});
  }catch(e){ res.status(500).json({error:e.message}); }
});
...
app.get('/api/trainer/earnings', trainerAuth, async(req,res)=>{
  try {
    ...
    for(const m of months){
      ...
      const paid_total=+((rec?.payments||[]).reduce((s,p)=>s+(+p.amount||0),0) || (rec?.status==='paid'&&!(rec?.payments||[]).length ? total : 0)).toFixed(2);
      rows.push({month:m, sessions:st.sessions, attendances:st.attendances, revenue:+st.revenue.toFixed(2),
        ...
        paid_total, outstanding:+Math.max(0,total-paid_total).toFixed(2), payments:rec?.payments||[],
...
// Body sa dávajú za úroveň, ktorú si klientka naozaj ZAPLATILA — darovaný upgrade
// (napr. Gold pre klientku mesiaca) bodovú úroveň nezvyšuje. Mapa user_id → posledná
// zaplatená úroveň z transakcií (free/darčekové sa nerátajú, kreditom platené áno).
...
// Efektívna úroveň pre body: zaplatená > (nedarovaný záznam) > default
const effectiveMemTier = (paidTier, recTier, isGift) => paidTier || (isGift ? null : recTier);
...
app.get('/api/client/spotlight', auth, async(req,res)=>{
  try {
    ...
    const memActive = {}, memName = {}, memTier = {};
    const paidTiersSpot = await paidMembershipTierMap();
  ...
  try{
    ...
    const out=schools.map(s=>{
      const cls=classes.filter(c=>c.school_id===s._id).map(c=>{
        ...
        const parents=users.filter(u=>u.venceky_class_id===c._id && u.venceky_role==='parent');
    ...
    const pendingUsers=(await q.find(db.users,{})).filter(u=>u.vencek_pending_role).map(u=>({
      id:u._id,name:u.name,email:u.email,role:u.vencek_pending_role,
      school:(schools.find(x=>x._id===u.vencek_pending_school_id)||{}).name||'?',
      class:(classes.find(x=>x._id===u.vencek_pending_class_id)||{}).name||''}));
    res.json({ok:true, pending:pendingUsers, schools:out, totals:tot, levels:VENCEK_LEVELS});
  }catch(e){ res.status(500).json({error:e.message}); }
});
...
// ── Admin: schváliť/zamietnuť čakajúceho učiteľa/riaditeľa (registrácia cez QR kód) ──
app.post('/api/admin/venceky/approve', adminAuth, async(req,res)=>{
  try{
    const u=await q.one(db.users,{_id:String(req.body.user_id||'')});
    if(!u||!u.vencek_pending_role) return res.status(404).json({error:'Používateľ nečaká na schválenie'});
    if(req.body.approve){
      const set={venceky_role:u.vencek_pending_role, venceky_school_id:u.vencek_pending_school_id};
      if(u.vencek_pending_role==='teacher') set.venceky_class_id=u.vencek_pending_class_id;
      await q.update(db.users,{_id:u._id},{$set:set,
        $unset:{vencek_pending_role:true,vencek_pending_class_id:true,vencek_pending_school_id:true}});
      await q.insert(db.notifications,{user_id:u._id,type:'venceky',
        title:'✅ Prístup schválený — Fusion Venčeky',
        body:'Váš prístup k venčekovému prehľadu je aktívny. Darček od nás: kupón VENCEKRODIC = 1. mesiac Zumby ZADARMO. 💛',
        read:false, created_at:nowISO()}).catch(()=>{});
    }else{
      await q.update(db.users,{_id:u._id},{$unset:{vencek_pending_role:true,vencek_pending_class_id:true,vencek_pending_school_id:true}});
      await q.insert(db.notifications,{user_id:u._id,type:'venceky',
        title:'Venčeky — prístup neschválený',
        body:'Prístup k venčekovému prehľadu sme zatiaľ neaktivovali. Ak ide o omyl, ozvite sa nám na fusionacademysk@gmail.com.',
        read:false, created_at:nowISO()}).catch(()=>{});
    }
    res.json({ok:true});
  }catch(e){ res.status(500).json({error:e.message}); }
