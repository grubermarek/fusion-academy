...
- Pasca: zvuk v telefóne sa odomkne len ťuknutím (preto úvodná obrazovka „Zapnúť zvuk"); stránka drží obrazovku cez Wake Lock.
- **Doplnené 22. 9. popoludní:** push na zamknutý telefón (`web-push`, VAPID kľúče v `settings.vapid_keys` — nie v gite; odbery `db.vencek_vecer_push` len z FCM/Apple/Mozilla/Windows; „O N min" a „čas vypršal" plánuje server `naplanuj`, lebo zamknutá stránka nebeží; iPhone len zo stránky pridanej na plochu — manifest `/api/vecer/manifest?t=` otvorí odkaz toho člena; SW `push`/`notificationclick` v `public/sw.js`, keď je stránka na očiach, notifikácia je tichá). Poradie párov `pary.set` (mená detí vidí len admin, riadiaci, moderátor, lektor, diplomy — `vidiMena`), diplomy a scenár na tlač `/vecer/<token>/tlac?co=diplomy|scenar` (`public/vecer-tlac.html`), honoráre do nákladov `team.costs` + zmena ceny upraví/zmaže náklad, položky `na_ziaka` (počet = žiaci skupiny), pripomienky 14/7/2/0 dní pred večerom v `runDailyTick` (`VENCEK_VECER.pripomienky`, ručne `POST /api/admin/vecer/pripomienky`). QA: `PUSH_FAKE=1` (šifruje, neposiela), `VECER_MINUTA_MS`.

### 7.10 Denný plánovač akcií a kampaní ✅ HOTOVO (2026-09-22, planovac.js, qa/planovac.test.js)
- Admin → Marketing → **Plánovač akcií** (sekcia `s-planovac`) + karta „Na čo sa pripraviť" v Dnes. Kalendár je deterministický, nič sa negeneruje LLM-kom.
- Zdroje udalostí: štátne sviatky a dni pracovného pokoja SR (pohyblivé cez Veľkú noc — anonymný gregoriánsky algoritmus; 1. 9., 28. 10. a 17. 11. sú sviatky, ale PRACOVNÉ dni), marketingové dni (Valentín, MDŽ, Deň tanca, Deň matiek = 2. májová nedeľa, MDD, Halloween, Black Friday = piatok po 4. novembrovom štvrtku, Cyber Monday, Mikuláš), sezónne okná (novoročná vlna, jarná vlna, septembrový nábor, vianočné darčeky, letný útlm, venčeková sezóna), školské prázdniny (**nastavenie** `settings.planovac_config.prazdniny`, predvyplnené 2026/2027 pre Banskobystrický kraj — každý rok ich treba overiť na minedu.sk), predĺžené víkendy (zlepené bloky voľna ≥ 3 dni) a interné akcie z DB (`ev_events`, `vencek_vecery`, koniec mesiaca).
- Ku každej príležitosti playbook: ponuka, publikum, kanály + kroky prípravy s `d` = dní vopred. Stav krokov a skryté položky v `db.planovac` (`_k:'stav'`), vlastné akcie tamtiež (`_k:'akcia'`).
- Sviatok pozná dopad na rozvrh: nájde prezenčné hodiny, ktoré naň pripadajú (online sa nerátajú — časť online hodín má kategóriu „Zumba“ a rozlíši ich len `location==='Online'`), a povie, koľko z nich ešte nie je zrušených.
